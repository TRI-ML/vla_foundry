...
> **Note:** For CI tests to pass on your fork, you may need to add your Hugging Face token. See [HF token setup for forks](#hf-token-setup-for-forks) below.
...
> **Note:** The `uv.lock` file tracks exact dependency versions for reproducible builds across machines. Always commit it after adding or removing dependencies.
...
> **Tip:** Set up your editor with a hotkey for `ruff format` to keep code consistently formatted as you work.
...
![HF Token Settings](assets/hf_key_screenshot.png)

> **Note:** The tests use public HuggingFace models (e.g., SmolVLM2-256M). No gated model access is required, but an `HF_TOKEN` is still needed for authenticated downloads.
